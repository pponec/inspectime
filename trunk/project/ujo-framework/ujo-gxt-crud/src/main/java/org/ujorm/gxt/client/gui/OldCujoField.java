...
public class OldCujoField extends AdapterField {
    ...
    public OldCujoField(TablePanel tablePanel) {
        ...
        textField.addListener(Events.OnKeyDown, new Listener<FieldEvent>() {
            @Override
            public void handleEvent(FieldEvent fe) {
                final boolean DEBUG = false;
                if (DEBUG) {
                    String msg = "Log ComponentEvent : " + fe.getEvent().getKeyCode();
                    GWT.log(msg, null);
                }
            ...
            protected void stop(FieldEvent fe) {
                fe.stopEvent();
                fe.setCancelled(true);
            }
        });

//        textField.addListener(Events.OnFocus, new Listener<FieldEvent>() {
//            @Override
//            public void handleEvent(FieldEvent be) {
//                button.focus();
//            }
//        });
        ...
        textField.addListener(Events.OnBlur, new Listener<FieldEvent>() {
            @Override
            public void handleEvent(FieldEvent be) {
                refreshValue();
            }
